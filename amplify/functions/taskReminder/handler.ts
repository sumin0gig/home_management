...
import { env } from '$amplify/env/task-reminder';
...
export const handler = async (): Promise<void> => {
  ...
  const dueTasks = await listAll(nextToken =>
    client.models.Task.listTaskByNextDueDate(
      { nextDueDate: today },
      { nextToken },
    ),
  );
  ...
  const roomIds = [...new Set(dueTasks.map(task => task.roomId))];
  ...
  const userIdToTaskTitles = new Map<string, string[]>();
  dueTasks.forEach(task => {
    const familyId = roomIdToFamilyId.get(task.roomId);
    if (!familyId) {
      return;
    }
    const userIds = familyIdToUserIds.get(familyId) ?? [];
    userIds.forEach(userId => {
      const titles = userIdToTaskTitles.get(userId) ?? [];
      titles.push(task.title);
      userIdToTaskTitles.set(userId, titles);
    });
  });

  const userIds = [...userIdToTaskTitles.keys()];
